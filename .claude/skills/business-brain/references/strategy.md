...
| **R2** | **5 missing Vercel env vars** for Pricing.tsx Payment Links (`VITE_STRIPE_*_URL`) — fresh deploy = Path B silently broken. | Add to `.env.example` and document in deploy runbook. Then verify present in Vercel for every env (preview + prod). |
